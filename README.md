...
| **Cloud path** | Debezium Server → **AWS Lambda** (same detector code, stateless price check) → Parquet on **Amazon S3**. `aws/deploy.sh kinesis` routes through an **AWS Kinesis** stream + event source mapping; `aws/deploy.sh http` (AWS Free plan) posts to a token-protected Lambda function URL; `aws/deploy.sh teardown` removes everything |
...
**AWS**: `aws login`, then `bash aws/deploy.sh` (Free plan: Lambda function URL) or `bash aws/deploy.sh kinesis` (paid plan),
`docker compose --profile aws up -d debezium-server`, watch `aws logs tail /aws/lambda/mkt-trust-processor --follow`
and `aws s3 ls s3://mkt-trust-lake-<account>-<region>/raw/ --recursive`, then `bash aws/deploy.sh teardown`.
